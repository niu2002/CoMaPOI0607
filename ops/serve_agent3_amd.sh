...
export OP_STR="${OP_STR:-${OP_STR_AGENT3:-amd-agent3-ca-v1}}"
...
resolve_adapter_path() {
  AGENT_TYPE_FOR_PATH="agent3" PROJECT_ROOT="$PROJECT_ROOT" "$PYTHON_BIN" - <<'PY'
import os
...
if [[ ! -d "$AGENT3_ADAPTER_PATH" ]]; then
  ...
  echo "[agent3-serve] train first with: bash ./ops/train_agent3_amd.sh"
  exit 1
fi

echo "[agent3-serve] agent1=base:$SERVED_MODEL_NAME"
echo "[agent3-serve] agent2=base:$SERVED_MODEL_NAME"
echo "[agent3-serve] agent3=LoRA:$AGENT3_ADAPTER_PATH"

"$SCRIPT_DIR/serve_agents_amd.sh"

echo "[agent3-serve] forward env:"
